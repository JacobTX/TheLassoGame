...
class MovingObject :
 ...
 MovingObject(double speed, double angle_deg, double argax, double argay, bool argpaused, bool rtheta) : Sprite() {
  ...
  void add_vx(double argvx) { vx += argvx;} /*To add horizontal speed*/
  void set_vy(double argvy) { vy = argvy; }
  void add_vy(double argvy) { vy +=argvy; }/*To add vertical speed*/
  void set_ax(double argax) { ax = argax; }
  void set_ay(double argay) { ay = argay; }

  double getXPos();
  double getYPos();
  double getXVel(){ return vx;} /* Returns x component of velocity */
  double getYVel(){ return vy;} /* Returns y component of velocity */
  double getVel(){return sqrt(pow(vx,2)+pow(vy,2));}/* Returns magnitude of net velocity */
